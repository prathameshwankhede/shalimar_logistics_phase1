  ...
  rawReqs.forEach(parentReq => {
    ...
    childItems.forEach((item, idx) => {
      ...
      const isWinningTransporter = Boolean(
        ...
        transMatchIds.some(tid => tid === String(winningTransporterId) || tid.toLowerCase() === String(winningTransporterId).toLowerCase())
      );

      if (fixedRateVal !== null) {
        isFixedRate = isWinningTransporter;
      }
      ...
      openReqs.push({
        ...
        is_awarded_to_other: Boolean(fixedRateVal !== null && !isWinningTransporter),
        winning_transporter_id: winningTransporterId,
        fixed_rate: fixedRateVal,
        requires_new_bid: !isFixedRate && fixedRateVal === null
      });
    });
  });
  return openReqs;
...
function dispatchTruck(dbState, reqId, itemId, loadedQty, authTransporter) {
  ...
  if (!finBid) throw new Error('RATE_NOT_FINALIZED');

  // Winning transporter verification
  ...
  if (!isWinner) {
    const err = new Error('TRANSPORTER_NOT_AUTHORIZED_FOR_DISPATCH');
    err.code = 'TRANSPORTER_NOT_AUTHORIZED_FOR_DISPATCH';
    err.status = 403;
    throw err;
  }
...
it('TEST 1: 200 MT @ 55, dispatch 150 -> remaining 50', () => {
  ...
  const res = dispatchTruck(db, 'req_200', 'item_200_01', 150, { id: 'trans_A' });
...
it('TEST 2: Remaining 50 visible to original winning transporter with dispatch enabled', () => {
  const db = create200MtScenario();
  dispatchTruck(db, 'req_200', 'item_200_01', 150, { id: 'trans_A' });
...
it('TEST 3: Non-winning transporter B sees item as Awarded to Other (dispatch disabled)', () => {
  const db = create200MtScenario();
  dispatchTruck(db, 'req_200', 'item_200_01', 150, { id: 'trans_A' });
  const rawReqs = [{ ...db.transport_requirements[0], items: db.transport_requirement_items }];
...
it('TEST 4: Winning transporter sees fixed rate ₹55', () => {
  const db = create200MtScenario();
  dispatchTruck(db, 'req_200', 'item_200_01', 150, { id: 'trans_A' });
  const rawReqs = [{ ...db.transport_requirements[0], items: db.transport_requirement_items }];
  const viewA = buildOpenRequirements(rawReqs, db, { id: 'trans_A' });
...
it('TEST 6: No new rate submission allowed (requires_new_bid = false)', () => {
  ...
  const rawReqs = [{ ...db.transport_requirements[0], items: db.transport_requirement_items }];
  const viewB = buildOpenRequirements(rawReqs, db, { id: 'trans_B' });
  assert.strictEqual(viewB[0].requires_new_bid, false);
});

it('TEST 7: Non-winning Transporter B attempting dispatch is rejected with 403', () => {
  const db = create200MtScenario();
  dispatchTruck(db, 'req_200', 'item_200_01', 150, { id: 'trans_A' });
  assert.throws(() => {
    dispatchTruck(db, 'req_200', 'item_200_01', 20, { id: 'trans_B' });
  }, (err) => err.code === 'TRANSPORTER_NOT_AUTHORIZED_FOR_DISPATCH');
});
...
it('TEST 8: Total cannot exceed 200 MT (Over-allocation rejected)', () => {
  ...
  dispatchTruck(db, 'req_200', 'item_200_01', 150, { id: 'trans_A' });
  assert.throws(() => {
    dispatchTruck(db, 'req_200', 'item_200_01', 55, { id: 'trans_A' }); // 150 + 55 = 205 > 200
...
it('TEST 12: /01 dispatch never affects /02', () => {
  const db = {
    ...
    rate_submissions: [{ id: 's1', requirement_id: 'req_batch', item_id: 'item_01', transporter_id: 'trans_A', final_rate: 50, is_finalized: 1 }],
    truck_dispatches: []
  };

  dispatchTruck(db, 'req_batch', 'item_01', 80, { id: 'trans_A' });
...
it('TEST 15: Integrity endpoint correctly detects inconsistencies', () => {
  ...
  const dbInconsistent = {
    transport_requirement_items: [{ id: 'i2', sub_indent_no: 'REQ-2/01', quantity_mt: 100, dispatched_quantity_mt: 30, remaining_quantity_mt: 50 }],
    truck_dispatches: [{ requirement_item_id: 'i2', loaded_quantity_mt: 40 }]
  };
