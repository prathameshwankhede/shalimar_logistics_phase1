...
async function runIndependentBidInputsTest() {
  ...
  // 2. Create FRESH Requirement Batch (REQ-0005) to test 0-quote initial state
  const createRes = await fetch(`${BASE_URL}/api/requirements`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
    body: JSON.stringify({
      req_no: 'SNPL/26-27/REQ-0005',
      pickup_origin: 'Katol',
      drop_location: 'Yerla',
  ...
  // Initial Check: Both bid inputs are empty (0 quotes)
  const initialRates1 = await (await fetch(`${BASE_URL}/api/requirements/${targetReq.id}/rates?item_id=${item1.id}`, { headers: { 'Authorization': `Bearer ${token}` } })).json();
  const initialRates2 = await (await fetch(`${BASE_URL}/api/requirements/${targetReq.id}/rates?item_id=${item2.id}`, { headers: { 'Authorization': `Bearer ${token}` } })).json();
  console.log(`INITIAL STATE: /01 count = ${initialRates1.count}, /02 count = ${initialRates2.count} (Both EMPTY ✅)`);

  // 3. Step A: Submit ₹77 ONLY for Sub-Indent /01
  console.log('\n👉 STEP 1: Submit ₹77 ONLY for Sub-Indent /01...');
  ...
  // 5. Step C: Resubmit /01 from ₹77 to ₹75
  ...
  // 6. Step D: Test "Submit All Batch Bids" behavior
  console.log('\n👉 STEP 4: Test Submit All Batch Bids (/01 = ₹75, /02 = ₹88)...');
  await fetch(`${BASE_URL}/api/rate-submissions`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
    body: JSON.stringify({
  ...
  const rates1Final = await (await fetch(`${BASE_URL}/api/requirements/${targetReq.id}/rates?item_id=${item1.id}`, { headers: { 'Authorization': `Bearer ${token}` } })).json();
  const rates2Final = await (await fetch(`${BASE_URL}/api/requirements/${targetReq.id}/rates?item_id=${item2.id}`, { headers: { 'Authorization': `Bearer ${token}` } })).json();

  const final1Rate = rates1Final.rates[0]?.rate_per_mt;
  const final2Rate = rates2Final.rates[0]?.rate_per_mt;

  console.log(`  • Final /01 Rate in MySQL: ₹${final1Rate}/MT`);
  console.log(`  • Final /02 Rate in MySQL: ₹${final2Rate}/MT`);

  const passCondition = 
    initialRates1.count === 0 && initialRates2.count === 0 &&
    ...
    final1Rate === 75 && final2Rate === 88;

  console.log('\n==================================================');
  if (passCondition) {
    console.log(`REQ-0005/01 → 300 MT → ₹75`);
    console.log(`REQ-0005/02 → 200 MT → ₹88\n`);
    console.log(`MYSQL:\nitem /01 → ₹75\nitem /02 → ₹88\n`);
    console.log('PASS.');
  } else {
    console.log('FAIL');
